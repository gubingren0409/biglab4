...
int main()
{
    ...
    // --- 测试 Block (Data Bitmap) ---
    // 这里的参数应该传 1 (Data) 而不是 0

	for (int i = 0; i < NUM; i++)
		block_num[i] = syscall(SYS_alloc_block);

	syscall(SYS_flush_buffer, 8);
	syscall(SYS_show_bitmap, 1); // [FIX] 0 -> 1 (查看 Data Bitmap)

	for (int i = 0; i < NUM; i+=2)
		syscall(SYS_free_block, block_num[i]);
	
	syscall(SYS_flush_buffer, 8);
	syscall(SYS_show_bitmap, 1); // [FIX] 0 -> 1
    ...
    // --- 测试 Inode (Inode Bitmap) ---
    // 这里的参数应该传 0 (Inode) 而不是 1

	for (int i = 0; i < NUM; i++)
		inode_num[i] = syscall(SYS_alloc_inode);

	syscall(SYS_flush_buffer, 8);
	syscall(SYS_show_bitmap, 0); // [FIX] 1 -> 0 (查看 Inode Bitmap)

	for (int i = 0; i < NUM; i++)
		syscall(SYS_free_inode, inode_num[i]);

	syscall(SYS_flush_buffer, 8);
	syscall(SYS_show_bitmap, 0); // [FIX] 1 -> 0
